...
class Tetris:
    ...
    def main_tetris(self, tetris_main_window):
        ...
        while True:
            ...
            if last_position_of_the_playing_field:
                print('\a')
                tm = Timer(1.5, self.clear_window, args=[tetris_main_window, 5 + self.tetris_board_properties.board_depth + 1])
                tm.start()
                last_position_of_the_playing_field = False
                self.tetris_board_properties.play_field = np.logical_or(self.tetris_board_properties.play_field, self.tetris_board_properties.overlay_of_play_field)
                refreshed_tetris_objects = Tetris()
    ...
    @staticmethod
    def clear_window(tetris_main_window, position: int):
